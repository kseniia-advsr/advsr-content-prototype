...
export default function App() {
  // Identifies one capture/waitlist Supabase row: every capture call and the
  // final waitlist submission for a given generation session accumulate onto
  // this same id. Regenerated by resetSession on "+ New content" so a second
  // round of generation starts its own row instead of overwriting the first.
  const [sessionId, setSessionId] = useState(() => crypto.randomUUID());
  const [state, setState] = useState(initialState);
  // True once the welcome screen, tone dialog, and insights funnel have all
  // been completed at least once this tab. A ref (not state) so it survives
  // resetSession's setState calls — "+ New content" checks it to decide
  // whether to re-run onboarding from scratch or skip straight back to the
  // composer under the same tone profile.
  ...
  const resetSession = () => {
    ...
    if (hasCompletedOnboardingRef.current) {
      // Onboarding already happened once this tab — don't re-show the
      // welcome screen or re-run the tone dialog/funnel, and keep the tone
      // profile the advisor already gave us. Just clear the chat/generation
      // state so they get a clean composer for the next topic.
      setState((prev) => ({
        ...initialState(),
        welcomeOpen: false,
        toneOpen: false,
        toneResponses: prev.toneResponses,
      }));
    } else {
